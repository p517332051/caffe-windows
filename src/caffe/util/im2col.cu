...
namespace caffe {
	__device__ void caffe_gpu_memcpy_(const size_t N, const void* X, void* Y) {
		if (X != Y) {
			//cudaMemcpy(Y, X, N, cudaMemcpyDefault);  // NOLINT(caffe/alt_fn)
		}
	}
template <typename Dtype>
__global__ void im2col_gpu_kernel(const int n, const Dtype* data_im,//data_im [5 5]
    const int height, const int width, const int kernel_h, const int kernel_w,//height 5 width 5
    const int pad_h, const int pad_w,										 //kernel_h 3  kernel_w 3
    const int stride_h, const int stride_w,                                  //stride_h 1  stride_w 1 
    const int dilation_h, const int dilation_w,								//dilation_h 1 dilation_w 1 
    const int height_col, const int width_col,								//height_col 3   width_col 3
    Dtype* data_col) {													   //data_col [3 3]
  CUDA_KERNEL_LOOP(index, n) {											   //index = channel * height_col * width_col
    const int h_index = index / width_col; //channel * height_col
	...
	caffe_gpu_memcpy_(1,data_im, data_col);

	//cudaMemcpy(data_im, data_col, 1, cudaMemcpyDefault);
